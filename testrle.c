...
VD frameerrorhead() {
  framehtmlheader();
  LONF("<html><head><title>RLE test</title></head>");
  LONF("<body style='font-weight: bold; color: #e00;'>");
}
...
// -------------------- RECV ----------------------------------------------------------------
  ...
  CS contentval = NULL; // use $1 if no CONTENT_LENGTH
  IN inputlen = -1; // read stdin if no contentval
  IF (contentlen) {
    inputlen = 0;
    ...
    contentval = $1;
    inputlen = STRLEN(contentval);
  } EF (INPUTPIPE) {
    inputlen = -1; // read stdin until EOF
  } EL { inputlen = -2; } // no input
//  } // EL read stdin
//Cs("recv");
/*
  IF (0) {
...
// for now just send one -- need to remember all indexes and counts
  ...
  WI (inputlen LT 0 OR (INC inputi LT inputlen)) {
    IF (inputlen EQ -2) { BK; } // NO INPUT
    IN ch = NUL;
    IF (contentval AND (inputi GQ 0)) {
      ch = contentval[inputi];
    } EL { ch = GETFCH(stdin); }
//    ILOG1("/V:/$@one-", "%c", ch);
// ^^^^^^^^^^^^
// need to open pipe for writing.. then wait for reader..


// todo: frame0 to communicate as main user (user 0)
// user0: -> frame0 "syntax"...
// syntax: c=..&n=..&k=..&m=..
//Y1(" (%d)", ch);
    IF (ch EQEOF) { BK; }
...
// which pipe? use dns/dnc cycling codes to reuse allocated pipe connection
// or instead of pipe - follow status bit and write file cache
// need javascript rle decoder in index.html (frame parent)
  CHDIRORFAIL(RAMDIR) { LOG("chdirramdir"); RT 0; }
  CS testrlepath = "=1.rle";
  CS donerlepath = "+1.rle";
  WI (waitforcacheexistence(testrlepath, 0) EQ 'c') {
    WAIT10MS; // 100fps limiter !
//    LOGF("...");
  } THEN FS rle = OPENFILE(testrlepath);
  // read the file, write the file
  IF (!rle) { frameerror("cache open fail"); RT 0; }
/*  // 
 ...
 *   /
   // no fi and fo in testrle - read the cache, rename the cache - -> = -> +
    continue.... all old ... but later write the inputs to a fo stream !
    and maybe even read input responses from a fi stream if one is available
//Cs("opened");
...
//            Cs("coderead");
  ...
  */
  ...
  CH rlereply = (rle) ? 1 : 0; // rle is true from earlier - means it's open!
  IF (rlereply) { // YES, until PNG is also an option
    framehtmlheader();
    LOGF("<html><head>\n");
    LOGF("  <script type=\"text/javascript\">\n");
    // write data object to header rather than body
    LOGF("  var data = { code: '%s',\n", replycode);
     // ...file will be streamed
    LOGF("     unhandledinput: '%d',\n", INPUTPIPE);
    LOGF("               data: '");
    IN readbytes = 0;
    CH readbuf[2]; // extra for padding
...
//    WI (bytesread LT countbuf) { <-- from pipe, expected data length is given
// in this case, data is from file ................ BK on EOF
    LOOP {
// open non-blocking later? as fd or file ?
//      readbytes = read(fo, readbuf, 1);
//      IN readbytes = 0;
      IN ch = GETFCH(rle);
      IF (ch EQEOF) { readbytes = 0; } // was errno set? use -1 instead?
      EL { readbytes = 1; readbuf[0] = ch; }
      IF (readbytes EQ 0) {
//        Ys("DATAWAIT");
...
//        sched_yield();
    //    Ys("DATAEOF");
        ...
        // later when nonblocking
        IF (errno EQ EAGAIN) {
     //     WAIT50MS; // 20fps limit !! !! !! !!
          WAIT10MS; // presumably this is only for later with nonblocking mode!
//          Cs("DATAWAIT");
          readbytes = 0; // non-blocking, try again
//          sched_yield(); // give processor some time
        } EL { LOGF("[read fail]"); BK; } // bad error
//            R1("READDATAERR: %d", errno);
//            WAIT500MS;
      } EL { // assume readbytes is 1, readbuf[0] is ch
 ...
 //   EF (bbi GT 0) // part of a triplet remains
    IF (bbi GT 0) // part of a triplet remains
      { framelogbase(basebuf, bbi); }
    LOGF("',\n");
    CLOSEFILE(rle);
    IF (rename(testrlepath, donerlepath) NQ 0) {
      LOG2F("               move: 'rename errno: %d, errstr: %s',\n", errno, strerror(errno));
    } EL { LOGF( "               move: 'done',\n"); }
