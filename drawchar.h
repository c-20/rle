...
FP drawsomechars(CCS srcfile, IN ptsize, FP x, FP y, CCS textstr, CH visible) {
  ...
  WI ((ss = nextutf8char(ss, &character)) NQNULL) {
    IF (character LT ' ') { // write control characters in smalltext
      CH controlstr[10];
      STRF(controlstr, "(%d)", character);
      xx += drawsomechars(srcfile, ptsize / 4, xx, y, controlstr, 1);
      CT; // next char
    }
    FPP fpv = NULL;
    UCHP fiv = NULL;
...
FP drawlineswh(CCS srcfile, IN ptsize, FP x, FP y, CS str, FPP w, FPP h) {
  ...
  WI (str[0] NQNUL) {
    CS endp = str;
    WI (*endp NQNUL AND NQ2(*endp, '\r', '\n')) // \r replaces \n (match both)
      { INC endp; }
    FP thiswidth = 0.0f;
    IFEQ2(*endp, '\r', '\n') { // \r replaces \n on keyboard ... '\n') { // line
      CH endpch = *endp; // remember which it was
...
// can delay or animate each character
      xx += (minwidth LT 0.0f) ? lineheight : (offset LT minwidth) ? minwidth : offset;
    } EL {
      colix = 0; INC rowix;
      xx = x; yy -= (minheight GT lineheight) ? minheight : lineheight;
