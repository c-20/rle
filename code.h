...
// ^ this should probably link to IFNQAND...
...
#define NQ2(v, n1, n2)                 (v NQ n1 AND v NQ n2)
#define NQ3(v, n1, n2, n3)             (v NQ n1 AND v NQ n2 AND v NQ n3)
...
#define OPENBLOCKFILE(f)  OPENBINARYFILE(f)
#define SAVEBINARYFILE(f) fopen(f, "wb")
#define SAVEBLOCKFILE(f)  SAVEBINARYFILE(f)
...
#define CLOSEBLOCKFILE(f)  CLOSEBINARYFILE(f)
...
// CMDLINE defines a FS and an output CS. It runs the
//  given cmdstr using the FS and writes the first line to CS
// -> if CS length exceeded, writes CMDLINE OVERFLOW to CS
// -> if FS fails to open, writes CMDLINE EXECFAIL to CS
#define CMDLINEOUTLEN    1024
#define FSLINEOUTLEN     CMDLINEOUTLEN
#define CMDLINE(cmdfs, cmdlineout, cmdstr)               \
    CH cmdlineout[CMDLINEOUTLEN];                        \
    cmdlineout[0] = NUL;                                 \
    FS cmdfs = OPENCMD(cmdstr);                          \
    IF (cmdfs) {                                         \
      CS cmdfs##c = cmdlineout;                          \
      CS cmdlineout##lim = cmdfs##c + CMDLINEOUTLEN;     \
      LOOP {                                             \
        IF (cmdfs##c GQ cmdlineout##lim)                 \
          { STRF(cmdlineout, "-CMDLINE OVERFLOW"); BK; } \
        IN ch = GETFCH(cmdfs);                           \
        IF (ch EQEOF) { BK; }                            \
        EF (ch EQ '\n') { BK; }                          \
        EL {                                             \
    ...
    } EL { STRF(cmdlineout, "-CMDLINE EXECFAIL"); }
#define FSLINE(cmdfs, fslineout)                       \
    CH fslineout[CMDLINEOUTLEN];                       \
    fslineout[0] = NUL;                                \
    IF (cmdfs) {                                       \
      CS cmdfs##c = fslineout;                         \
      CS fslineout##lim = cmdfs##c + CMDLINEOUTLEN;    \
      LOOP {                                           \
        IF (cmdfs##c GQ fslineout##lim)                \
          { STRF(fslineout, "-FSLINE OVERFLOW"); BK; } \
        IN ch = GETFCH(cmdfs);                         \
        IF (ch EQEOF) { BK; }                          \
        EF (ch EQ '\n') { BK; }                        \
        EL {                                           \
    ...
    } EL { STRF(fslineout, "-FSLINE READFAIL"); }
#define CSFSLINE(csline, cmdfs)  FSLINE(cmdfs, csline)

#define LOGCMD(fs, cmdstr, linebreak) \
  FS fs = OPENCMD(cmdstr);     \
  IF (fs) {                    \
    WI (1) {                   \
      IN ch = GETFCH(fs);      \
      IF (ch EQEOF) { BK; }    \
...
#define LONCMD(fs, cmdstr)  LOGCMD(fs, cmdstr, "\n")
#define LOBCMD(fs, cmdstr)  LOGCMD(fs, cmdstr, "<br>\n")
#define LOWCMD(fs, cmdstr, hasgw)  \
  IF (hasgw) { LOBCMD(fs, cmdstr); } EL { LONCMD(fs, cmdstr); }

#define LOBF(str)               LONF(str "<br>")
#define LOB1F(str, v1)          LON1F(str "<br>", v1)
#define LOB2F(str, v1, v2)      LON2F(str "<br>", v1, v2)
#define LOB3F(str, v1, v2, v3)  LON3F(str "<br>", v1, v2, v3)
#define LOWF(str, hasgw)  \
  IF (hasgw) { LOBF(str); } EL { LONF(str); }
#define LOW1F(str, v1, hasgw)  \
  IF (hasgw) { LOB1F(str, v1); } EL { LON1F(str, v1); }
#define LOW2F(str, v1, v2, hasgw)  \
  IF (hasgw) { LOB2F(str, v1, v2); } EL { LON2F(str, v1, v2); }
#define LOW3F(str, v1, v2, v3, hasgw)  \
  IF (hasgw) { LOB3F(str, v1, v2, v3); } EL { LON3F(str, v1, v2, v3); }

// LOGW assumes the variable 'gateway' is defined (as 0/NULL if no gateway)
#define LOGW(str)               LOWF(str, gateway)
#define LOGW1(str, v1)          LOW1F(str, v1, gateway)
#define LOGW2(str, v1, v2)      LOW2F(str, v1, v2, gateway)
#define LOGW3(str, v1, v2, v3)  LOW3F(str, v1, v2, v3, gateway)


// imported LOCK definitions from IN since -- requires them ---------------------------------------------
#define INLOCKFLAGS       O_RDONLY | O_CREAT | O_EXCL
#define INCREATELOCK(filename)     open(filename, INLOCKFLAGS)
#define INLOCKREADFLAGS   O_RDONLY
#define INLOCKWRITEFLAGS  O_WRONLY
#define INOPENLOCKREAD(filename)   open(filename, INLOCKREADFLAGS)
#define INCLOSELOCKREAD(fileid)    close(fileid)
#define INOPENLOCKWRITE(filename)  open(filename, INLOCKWRITEFLAGS)
#define INCLOSELOCKWRITE(fileid)   close(fileid)
#define INLOCKREADCHAR(fileid, writechar)         read(fileid, &writechar, 1)
#define INLOCKWRITECHAR(fileid, thischar)         write(fileid, &thischar, 1)
#define INLOCKWRITECHARS(fileid, thestr, thelen)  write(fileid, thestr, thelen)
//#define INLOCKWRITESTR(fileid, thestr, thelen)  \
//  INLOCKWRITECHARS(fileid, thestr, thelen); \
//  thestr[thelen] = NUL;
// ^ no need to write NUL since it will be within the buffer region, perhaps not at the end
#define INLOCKSEEKSTART(fileid)              lseek(fileid, 0, SEEK_SET)
#define INLOCKSEEKFROMSTART(fileid, more)    lseek(fileid, 0 + more, SEEK_SET)
#define INLOCKSEEKPREV(fileid)               lseek(fileid, -1, SEEK_CUR)
#define INLOCKSEEKNEXT(fileid)               lseek(fileid, 1, SEEK_CUR)
#define INLOCKSEEKEND(fileid)                lseek(fileid, 0, SEEK_END)
#define INLOCKSEEKFROMEND(fileid, less)      lseek(fileid, 0 - less, SEEK_END)

#define INNEWCHBUFFER(bufname, bufsize, bufchar)   \
  CH bufname[bufsize + 1];                         \
  CS bufname##lbi = bufname;                       \
  WI (bufname##lbi LT (bufname + bufsize))         \
    { *bufname##lbi = bufchar; INC bufname##lbi; } \
  *bufname##lbi = NUL;
// ------------------------------------------------------------------------------------------------------
...
#define CLEANMEM(type, n) (type *)calloc(sizeof(type), (n))
...
// CS ELEMENTS EQUAL
// #define CSEQ(dv, cs) .... need to know the length of CS without using a loop (unless defining a check variable that stops when non-matching (or at NUL)
...
// check usage scenarios: this should probably be CSQQ with CSQ meaning CS SEQ .. for now CSEQ
// later: #define CSQ(dv, cs) should check cs length and trigger the appropriate function
// ^ yeah that ....... use sizeof? how to get length of static string for precompiler ?.....................
// requires: string.h
...
#define CSENDP(endp, sp) CS endp = sp; WI (*endp NQNUL) { INC endp; }
#define CS_ENDP(ep, sp)  CSENDP(ep, sp)
#define STRENDP(ep, sp)  CSENDP(ep, sp)
...
#define LOG7F				printf
#define LOG8F				printf
#define LOG9F				printf // works with any number of params
...
#define LOGLCH(ix)			LOG1("%lc", ix); LOG0
...
#define Anewline	'\n'
#define Anl Anewline
#define Alinebreak     Anewline
// note that Alb is Aleftbracket
#define Atab		'\t'
...
#define RiR0(i)  R(i)
#define YiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Yi(i); } }
#define GiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Gi(i); } }
#define CiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Ci(i); } }
#define BiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Bi(i); } }
#define MiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Mi(i); } }
#define WiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Wi(i); } }
#define LiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Li(i); } }
#define DiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Di(i); } }
#define KiR0(i)  { IF (i EQ 0) { Ri(0); } EL { Ki(i); } }
...
#define R4(f, s1, s2, s3, s4)	RLOG4(f, s1, s2, s3, s4);
#define Y4(f, s1, s2, s3, s4)	YLOG4(f, s1, s2, s3, s4);
#define G4(f, s1, s2, s3, s4)	GLOG4(f, s1, s2, s3, s4);
#define C4(f, s1, s2, s3, s4)	CLOG4(f, s1, s2, s3, s4);
#define B4(f, s1, s2, s3, s4)	BLOG4(f, s1, s2, s3, s4);
#define M4(f, s1, s2, s3, s4)	MLOG4(f, s1, s2, s3, s4);
#define W4(f, s1, s2, s3, s4)	WLOG4(f, s1, s2, s3, s4);
#define L4(f, s1, s2, s3, s4)	LLOG4(f, s1, s2, s3, s4);
#define D4(f, s1, s2, s3, s4)	DLOG4(f, s1, s2, s3, s4);
#define K4(f, s1, s2, s3, s4)	KLOG4(f, s1, s2, s3, s4);
